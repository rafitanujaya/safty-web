...
import { Download, Scan, ShieldCheck, Bell, ArrowDown } from 'lucide-react';
...
export function HowItWorksSection() {
  ...
  const steps = [
    { num: '01', icon: Download, title: 'Install', accent: 'text-blue-500' },
    { num: '02', icon: Scan, title: 'Scan', accent: 'text-cyan-500' },
    { num: '03', icon: ShieldCheck, title: 'Neutralize', accent: 'text-emerald-500' },
    { num: '04', icon: Bell, title: 'Report', accent: 'text-amber-500' },
  ];

  return (
    <section ref={ref} id="product" className="bg-white py-32 md:py-40 px-6 md:px-12 lg:px-20 max-w-[1600px] mx-auto w-full">

      {/* Horizontal pipeline — NOT a grid */}
      <div className={cn(
        ...
        <div className="w-full flex flex-col md:flex-row items-center justify-center gap-0">
          {steps.map((step, i) => (
            <React.Fragment key={i}>
              ...
              {/* Connector line */}
              {i < steps.length - 1 && (
                <div className="hidden md:block w-16 lg:w-24 xl:w-32 h-[2px] bg-gradient-to-r from-slate-200 to-slate-100 mx-2" />
              )}
              {i < steps.length - 1 && (
                <div className="block md:hidden h-10 w-[2px] bg-slate-100" />
              )}
            </React.Fragment>
          ))}
        </div>
